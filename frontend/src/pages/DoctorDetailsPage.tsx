...
const DoctorDetailsPage: React.FC = () => {
  const rankSimilarDoctors = (baseDoctor: Doctor, doctors: Doctor[]) => {
    ...
    return ranked.filter((d) => d.score > 0).sort((a, b) => b.score - a.score);
  };
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const doctorRes = await axios.get(`${API_BASE}/api/doctors/${slug}`);
  ...
  const seoTitle =
    doctor?.seo_title || globalSEO?.seo_title || doctor?.name || "Best Doctor";
  ...
  const imageTitle =
    doctor.image_title || `${doctor.name} - ${doctor.speciality || "Doctor"}`;
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              <div className="lg:col-span-2  p-6 ">
                <div className="flex flex-col md:flex-row gap-6 items-start">
                  <div className="flex-shrink-0 w-full sm:w-[280px] md:w-[320px]">
                    <img
                      src={
                        doctor?.image_url
                          ? `${API_BASE}${doctor.image_url}`
                  ...
                  <div className="flex-1">
                    ...
                    <button
                      ...
                      className="mt-4 inline-block bg-[#F0A324] hover:bg-yellow-600 transition px-4 py-3 rounded-lg"
                      style={{
                ...
                <div className="mt-10">
                  <div
                    ...
                    dangerouslySetInnerHTML={{
                      __html: doctor.description_html,
                    }}
                  />
                </div>
        ...
        <section className="py-6 similar-doctor-section-docdetails-page">
          <Container>
            ...
            <div className="relative">
              <Slider
                {...doctorSlider}
                key={`${similarDoctors.length}-${isMobile}`}
              >
                  ...
                  <div key={d.id} className="px-3">
                    <Link to={`/doctor/${d.slug}`}>
                      <div className="bg-white rounded-xl shadow text-center ">
                        <img
                          src={
                            d.image_url && d.image_url.trim().length > 0
                      ...
                      <div key={h.id} className="px-3 related-cards-1">
                        <Link to={`/hospitals/${h.slug}`}>
                          <div className="rounded-xl overflow-hidden shadow">
                            <img
                              src={
                                h.image_url && h.image_url.trim().length > 0

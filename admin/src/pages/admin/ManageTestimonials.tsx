...
const ManageTestimonials: React.FC = () => {
  ...
  const API = ((import.meta as any).env?.VITE_BASE_URL || "").replace(/\/$/, "") + "/api/testimonials";
  ...
  const handleEdit = (t: Testimonial) => {
    ...
    if (t.image_url) {
      setPreviewImage(`${((import.meta as any).env?.VITE_BASE_URL || "").replace(/\/$/, "")}${t.image_url}`);
    }
            ...
            {testimonials.map((t) => (
              <tr key={t.id} className="border">
                <td className="p-3 border">
                    {t.image_url ? (
                    <img
                      src={`${((import.meta as any).env?.VITE_BASE_URL || "").replace(/\/$/, "")}${t.image_url}`}
